"""Pipeline de monitoramento acústico — processa os WAVs de audios/.

Para cada arquivo .wav, executa: análise no tempo, espectro (FFT) com
picos, espectrograma, filtros FIR/IIR e nível em dB(A) comparado à
NBR 10151 (limite diurno). Salva as figuras em resultados/ e imprime
uma tabela comparativa.

Como rodar (a partir da pasta codigo/):  python main.py
"""
...
def imprimir_tabela(nomes, niveis, situacoes):
    ...
    print("=" * largura)
    print(f"{'Arquivo':<24}{'dB(A)':<10}{'Situação'}")
    print("-" * largura)
...
def main():
    """Executa todas as etapas em cada áudio da pasta audios/."""
    ...
    for caminho in audios:
        ...
        # 2) Domínio da frequência (FFT + picos).
        ...
        # 4) Filtros FIR e IIR (resposta dos dois + sinal filtrado pelo IIR).
        ...
        # 5) Nível em dB(A) e comparação com a norma.
        nivel = proc.nivel_dba(sinal, fs)
        ...
        print(f"   nível: {nivel:.1f} dB(A) -> {situacao} do limite {limite}\n")
